defmodule Thunk.PreludeTest do
  ...
  test "fold and length", %{ctx: ctx} do
    assert ev(ctx, "fold(fn(acc, x) -> acc + x, 0, xs)", %{xs: [1, 2, 3]}) == 6
    assert ev(ctx, "fold(fn(acc, x) -> acc + x, 0, [])") == 0
  ...
  describe "list library" do
    ...
    test "map and filter keep order", %{ctx: ctx} do
      assert ev(ctx, "map(fn(x) -> x * x, xs)", %{xs: [1, 2, 3]}) == [1, 4, 9]
      assert ev(ctx, "map(fn(x) -> x, [])") == []
      assert ev(ctx, "filter(fn(x) -> x < 3, xs)", %{xs: [5, 1, 4, 2]}) == [1, 2]
    end

    test "take, drop and split_at", %{ctx: ctx} do
      ...
      assert ev(ctx, "split_at(1, xs)", %{xs: [1, 2, 3]}) == [[1], [2, 3]]
    end
  ...
  describe "parallel library" do
    @small "fn(v) -> length(v) < 3"

    test "pmap equals map", %{ctx: ctx} do
      xs = Enum.to_list(1..20)

      assert ev(ctx, "pmap(fn(x) -> x * 2, xs, #{@small})", %{xs: xs}) ==
               Enum.map(xs, &(&1 * 2))

      assert ev(ctx, "pmap(fn(x) -> x, [], #{@small})") == []
    end
    ...
    test "pmap and reduce give the same answer under the local scheduler", %{ctx: ctx} do
      ...
      assert ev(
               local,
               "reduce(add, pmap(fn(x) -> x * x, xs, #{@small}), #{@small})",
               %{xs: xs}
  ...
  describe "what the parallel library puts in a piece" do
    ...
    setup %{ctx: ctx} do
      ...
      def main_map(xs) = pmap(square, xs, small?)
      def main_reduce(xs) = reduce(add, xs, small?)
      """
    ...
    test "pmap does not capture the input list in its base case", %{rec: rec} do
      ...
      assert ev(rec, "main_map(xs)", %{xs: xs}) == Enum.map(xs, &(&1 * &1))
    ...
    test "reduce does not capture the input list in its base case", %{rec: rec} do
      xs = Enum.to_list(1..20_000)
      assert ev(rec, "main_reduce(xs)", %{xs: xs}) == Enum.sum(xs)
      assert_received {:work, work}
      assert piece_size(work) < 2_000
    end
  end
  ...
  describe "sorting" do
    test "insert and insertion_sort", %{ctx: ctx} do
      assert ev(ctx, "insert(2, xs)", %{xs: [1, 3]}) == [1, 2, 3]
      assert ev(ctx, "insert(0, [])") == [0]
      assert ev(ctx, "insertion_sort(xs)", %{xs: [3, 1, 2, 1]}) == [1, 1, 2, 3]
      assert ev(ctx, "insertion_sort([])") == []
    end

    test "merge_sorted", %{ctx: ctx} do
      assert ev(ctx, "merge_sorted(xs, ys)", %{xs: [1, 4, 9], ys: [2, 3, 10]}) ==
               [1, 2, 3, 4, 9, 10]

      assert ev(ctx, "merge_sorted([], ys)", %{ys: [1]}) == [1]
      assert ev(ctx, "merge_sorted(xs, [])", %{xs: [1]}) == [1]
      assert ev(ctx, "merge_sorted(xs, ys)", %{xs: [1, 1], ys: [1]}) == [1, 1, 1]
    end
  end

  describe "strings and counting" do
    test "split_words on codepoints", %{ctx: ctx} do
      assert ev(ctx, "split_words(chars(s))", %{s: "the  quick\nbrown\tfox "}) ==
               ["the", "quick", "brown", "fox"]

      assert ev(ctx, "split_words(chars(s))", %{s: ""}) == []
      assert ev(ctx, "split_words(chars(s))", %{s: "   "}) == []
      assert ev(ctx, "split_words(chars(s))", %{s: "città"}) == ["città"]
    end

    test "association lists", %{ctx: ctx} do
      al = [["a", 1], ["b", 2]]
      assert ev(ctx, "assoc_get(k, al, 0)", %{k: "b", al: al}) == 2
      assert ev(ctx, "assoc_get(k, al, 0)", %{k: "z", al: al}) == 0
      assert ev(ctx, "assoc_inc(k, 1, al)", %{k: "a", al: al}) == [["a", 2], ["b", 2]]
      assert ev(ctx, "assoc_inc(k, 5, al)", %{k: "c", al: al}) == [["a", 1], ["b", 2], ["c", 5]]

      assert ev(ctx, "assoc_merge(a, b)", %{a: al, b: [["b", 1], ["c", 1]]}) ==
               [["a", 1], ["b", 3], ["c", 1]]
    end

    test "count_words", %{ctx: ctx} do
      assert ev(ctx, "count_words(ws)", %{ws: ["a", "b", "a"]}) == [["a", 2], ["b", 1]]
    end
  end
end
